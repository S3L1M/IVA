...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\mohamedselim\\Desktop\\GSoC\\IVA\\Contrast Sensitivity\\staircase 4q\\iva_stair_4.py',
...
GA = visual.GratingStim(
    ...
    ori=0.0, pos=[0,0], size=(float(expInfo['stimulus size']), float(expInfo['stimulus size'])), sf=float(expInfo['spatial freq']), phase=0.0,
...
GA_loop = data.StairHandler(startVal=float(expInfo['max contrast']), extraInfo=expInfo,
    stepSizes=[0.4,0.4,0.2,0.2,0.1], stepType='lin',
    nReversals=0.0, nTrials=12.0, 
    nUp=1.0, nDown=2.0,
    minVal=float(expInfo['min contrast']), maxVal=float(expInfo['max contrast']),
    originPath=-1, name='GA_loop')
thisExp.addLoop(GA_loop)  # add the loop to the experiment
level = thisGA_loop = float(expInfo['max contrast'])  # initialise some vals
