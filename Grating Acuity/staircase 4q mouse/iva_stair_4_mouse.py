...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\mohamedselim\\Desktop\\GSoC\\IVA\\Grating Acuity\\staircase 4q mouse\\iva_stair_4_mouse.py',
...
for thisGA_loop in GA_loop:
    ...
    grating_acuityComponents = [text, GA, key_resp, etRecord]
    ...
    while continueRoutine:
        ...
        if text.status == STARTED:
            ...
            if tThisFlipGlobal > text.tStartRefresh + float(expInfo["stimulus duration"])-frameTolerance:
        ...
        if text.status == STARTED:  # only update if drawing
            text.setText(etRecord.pos, log=False)
        ...
        if etRecord.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > etRecord.tStartRefresh + float(expInfo["stimulus duration"])-frameTolerance:
                # keep track of stop time/frame for later
